...
pid=$(ps -fe | grep ${HELIDON_APP_NAME} | grep -v -e bash -e grep | awk '{print $2}')
if [ ! -z "$pid" ]; then
...
if [ $? -eq 0 ]; then
  echo "Helidon app is now running with pid $(ps -fe | grep ${HELIDON_APP_NAME} | grep -e bash -e grep | awk '{print $2}')!"
  break
fi
