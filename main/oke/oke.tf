...
#   oke_cluster_name                                                            = "oke-cluster${var.resource_name_suffix}"
...
resource "oci_containerengine_cluster" "oci_oke_cluster" {
  ...
  name               = "oke-cluster${var.resource_name_suffix}"  # var.oke_cluster_name
  # vcn_id             = oci_core_virtual_network.oke_vcn[0].id      # var.use_existing_vcn ? var.vcn_id : oci_core_vcn.oke_vcn[0].id
  vcn_id             = oci_core_virtual_network.oke_vcn.id      # var.use_existing_vcn ? var.vcn_id : oci_core_vcn.oke_vcn[0].id
  ...
  options {
    ...
    service_lb_subnet_ids = [oci_core_subnet.oke_lb_subnet.id] # [var.use_existing_vcn ? var.lb_subnet_id : oci_core_subnet.oke_lb_subnet[0].id]
